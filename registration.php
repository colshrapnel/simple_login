...
// Initialize all values, so we won't have to always check them for existence
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (strlen($password) < 8 || strlen($password) > 72) {
        $error['password'] = 'Please enter password, it must be from 8 to 72 characters long.';
    }
    ...
    if (implode($error) === '')
    {
        ...
        // after each successful POST request there MUST be a redirect
        header("Location: /index.php");
        // after sending Location header the code MUST be terminated
        die;
    }
}
// Now, in case there was no POST request
// Or it was but with errors
// Start the output.
// We have $error and $input arrays defined and filled above to be used in the templates
